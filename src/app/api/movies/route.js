...
export async function POST(request) {
  ...
  if (action === 'CREATE') {
    ...
    globalRooms[newCode] = {
      code: newCode,
      phase: 0, // 0 = Purge (10 movies), 1 = Shortlist (4 movies), 2 = Reveal
      movies: movies,
      survivingMovies: [],
      hostVetoes: [],
      guestVetoes: [],
  ...
  if (action === 'SUBMIT_VETO') {
    ...
    if (room.hostVetoes.length === 3 && room.guestVetoes.length === 3) {
      ...
      // Ensure exactly 4 make it to the shortlist
  ...
  // Phase 1: Submit 2 Shortlist picks
